...
describe('Booking tests are running', function() {
  ...
  it('should contain the room is associated with as a instance of the Room class', () => {
    testBooking.retrieveRoomInfo(testRoomData);
    assert.instanceOf(testBooking.roomDetails, Room);
  })
  
  it('should provide an error when a room is not found under that Booking within the given data', ()=> {

    let falseRoomData = [];
    assert.equal(testBooking.retrieveRoomInfo(falseRoomData), 'Error in finding room for selected booking');
  })
});